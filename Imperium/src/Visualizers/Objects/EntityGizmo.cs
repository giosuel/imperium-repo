...
{
    private EnemyParent entityController;
    ...
    private readonly Dictionary<string, int> VisualizerObjectNetIds = [];
    ...
    internal void Init(EntityGizmoConfig config, Visualization visualizer, EnemyParent entity)
    {
        entityConfig = config;
    ...
    )
    {
        ...
        if (!VisualizerObjects.TryGetValue(identifier, out var visualizer))
        {
            ...
            VisualizerObjectNetIds[identifier] = entityController.GetComponent<PhotonView>()?.ViewID
                                                 ?? entityController.GetInstanceID();
        }
    ...
    {
        Imperium.IO.LogError($"gizmo enemy: {entityController.Enemy}");
        Imperium.IO.LogError($"gizmo agent: {entityController.Enemy.NavMeshAgent}");
        Imperium.IO.LogError($"gizmo agent2: {entityController.Enemy.NavMeshAgent.Agent}");
        var corners = entityController.Enemy.NavMeshAgent.Agent.path.corners;
